...
import { Activity, Target, TrendingUp, Bell, Calendar, Utensils, Settings, ChevronRight, Play, Award, Flame, Menu, X, BarChart, Pause, SkipForward } from 'lucide-react';
...
const App = () => {
  ...
  // Estados para o sistema de séries
  ...
  // Efeito para o cronômetro do exercício
  useEffect(() => {
    let interval;
    if (isRunning && workoutPhase === 'working' && currentDay?.countType === 'time') {
      interval = setInterval(() => {
        setTimer(prev => prev + 1);
      }, 1000);
    }
    return () => clearInterval(interval);
  }, [isRunning, workoutPhase, currentDay]);

  // Efeito para o cronômetro de descanso
  useEffect(() => {
    let interval;
    if (isResting && workoutPhase === 'resting') {
      interval = setInterval(() => {
        setRestTimer(prev => {
          if (prev <= 1) {
  ...
  // Funções para o sistema de séries
  ...
  const completeSet = () => {
    ...
    if (currentSet < totalSets) {
      // Iniciar descanso para próxima série
      setWorkoutPhase('resting');
      setIsResting(true);
      setRestTimer(currentDay.rest);
    } else {
        ...
        <div className={`${cardBg} rounded-3xl p-6 w-full max-w-md`}>
          ...
          <div className="text-center mb-6">
            ...
            <div className="flex justify-center items-center gap-4 mb-4">
              <div className={`px-3 py-1 rounded-full ${workoutPhase === 'working' ? 'bg-green-500 text-white' : 'bg-gray-600 text-gray-300'}`}>
                Série {currentSet} de {totalSets}
              </div>
              <div className={`px-3 py-1 rounded-full ${workoutPhase === 'resting' ? 'bg-blue-500 text-white' : 'bg-gray-600 text-gray-300'}`}>
                Descanso
              </div>
            </div>
          </div>
          ...
          {workoutPhase === 'preparing' && (
            <div className="text-center">
              <h4 className="text-2xl font-bold text-orange-500 mb-4">Preparar para a Série {currentSet}</h4>
              <p className={textSec + " mb-6"}>Posicione-se corretamente para começar o exercício</p>
              <button 
                onClick={startExercise}
                className="w-full bg-green-500 text-white py-4 rounded-xl font-bold text-lg hover:bg-green-600 transition"
              >
                Começar Série {currentSet}
              </button>
            </div>
          )}
          ...
          {workoutPhase === 'working' && (
            <div className="text-center">
              <h4 className="text-xl font-bold text-green-500 mb-2">Série {currentSet} em Andamento</h4>
              ...
              {currentDay.countType === 'time' ? (
                ...
                <div className="text-center mb-6">
                  ...
                  <p className={textSec}>Repetições realizadas</p>
              ...
              <button 
                onClick={completeSet}
                className="w-full bg-red-500 text-white py-4 rounded-xl font-bold text-lg hover:bg-red-600 transition"
              >
                Finalizar Série {currentSet}
              </button>
            </div>
          )}
          ...
          {workoutPhase === 'resting' && (
            <div className="text-center">
              <h4 className="text-xl font-bold text-blue-500 mb-2">Descanso</h4>
              <div className="text-center mb-6">
                <div className="text-5xl font-bold text-blue-500 mb-2">{formatTime(restTimer)}</div>
                <p className={textSec}>Próxima série em</p>
                <p className="text-sm text-gray-400 mt-2">Série {currentSet + 1} de {totalSets}</p>
              </div>
          ...
          {workoutPhase === 'completed' && (
            <div className="text-center">
              <h4 className="text-2xl font-bold text-green-500 mb-4">Treino Concluído! 🎉</h4>
              <p className={textSec + " mb-6"}>Parabéns! Você completou todas as {totalSets} séries!</p>
              <button 
                onClick={() => {
                  setCurrentDay(null);
                  setWorkoutPhase('preparing');
